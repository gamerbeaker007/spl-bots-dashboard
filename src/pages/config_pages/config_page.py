...
layout = dbc.Container([
    dbc.Row([
        html.H1('Add management account'),
        ...
        dbc.Row(config_page_monitoring.get_layout()),
    ]),
])
...
def add_remove(add_clicks, account_name):
    ...
    if config_page_ids.add_button == ctx.triggered_id:
        if not config.read_only:
            ...
            if len(store_util.get_management_account_names()) > 0:
                text = 'Only one management account can be added'
            else:
...
def get_accounts(added, removed):
    current_account_names = store_util.get_management_account_names()
    return html.P('Current management account: ' + ', '.join(current_account_names))
