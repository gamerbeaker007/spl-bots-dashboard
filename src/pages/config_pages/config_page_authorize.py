...
def get_layout():
    layout = dbc.Row([
        html.H4('Add posting key for management account:', className='mt-5'),
        dbc.Col(
            dbc.InputGroup([
                dbc.InputGroupText('Account'),
                dcc.Dropdown(id=config_page_ids.posting_key_user_dropdown,
                             options=store_util.get_management_account_names(),
                             className='dbc',
                             style={"width": "70%"},
                             ),
            ], className='mb-3'),
...
def update_posting_key_user_list(added_trigger, removed_trigger):
    return store_util.get_management_account_name(), store_util.get_management_account_names()
